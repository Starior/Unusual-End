...
public class LeechingWandRightClickProcedure {
    ...
    private static void execute(PlayerInteractEvent.RightClickItem event, LevelAccessor world, double x, double y, double z, Entity entity) {
        if (entity == null)
            return;
        if ((entity instanceof LivingEntity _livEnt ? _livEnt.getMainHandItem() : ItemStack.EMPTY).getItem() == UnusualEndItems.LEECHING_WAND.get()) {
        ...
        } else if ((entity instanceof LivingEntity _livEnt ? _livEnt.getOffhandItem() : ItemStack.EMPTY).getItem() == UnusualEndItems.LEECHING_WAND.get()) {
            if (event != null) {
