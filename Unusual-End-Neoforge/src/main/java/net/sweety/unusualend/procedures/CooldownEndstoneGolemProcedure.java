...
public class CooldownEndstoneGolemProcedure {
    ...
    private static void execute(LivingIncomingDamageEvent event, LevelAccessor world, double x, double y, double z, DamageSource damagesource, Entity entity, Entity sourceentity, double amount) {
        ...
        if (sourceentity instanceof EndstoneGolemEntity) {
                ...
                if ((sourceentity instanceof EndstoneGolemEntity _datEntI ? _datEntI.getEntityData().get(EndstoneGolemEntity.DATA_push) : 0) == 0) {
                    ...
                    UnusualEnd.queueServerWork(4, () -> {
                        ...
                        for (int index0 = 0; index0 < 5; index0++) {
                            world.levelEvent(2001, BlockPos.containing(entity.getX(), entity.getY() + 1, entity.getZ()), Block.getId(UnusualEndBlocks.PURPUR_EMBEDDED_END_STONE.get().defaultBlockState()));
                        }
                    });
                    {
            ...
            if (damagesource.is(DamageTypes.CRAMMING)) {
                if (entity instanceof LivingEntity _entity && !_entity.level().isClientSide())
                    _entity.addEffect(new MobEffectInstance(UnusualEndMiscRegister.HEAVINESS, 40, 2, false, true));
